...
- `displayName`：可选，最多 80 字；纯昵称模板至少 6 字。
- `text`：可选，最多 320 字；话术模板至少 6 字。
- `displayName` 和 `text` 至少有一个达到上述最小长度。
- `category`：仅允许 `cn_adult_solicitation`、`adult_solicitation`、`remote_sample`。
- `enabled: false` 可临时停用某条样本。
- 最多读取 2000 条，重复文本会去重。
- 控制字符、双向文本控制符和 `< > { } \` 会被清洗。

样本只在帖子详情页评论区参与相似度判断。独立长话术命中要求至少 16 个有效字符且相似度不低于 94%；纯昵称模板要求昵称相似度不低于 94%，并且评论正文属于低信息内容。随机装饰表情不参与话术或昵称相似度计算。

## 明确不再使用

下列旧文件可保留作历史记录，但 v0.12.0 不会请求或读取：
